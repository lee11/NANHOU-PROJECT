...
def setIfPresent(dest, kw, item, default):
	if item in kw:
		setattr(dest, item, kw[item])
	else:
		setattr(dest, item, default)
		
#disjoint, brute force collision detection between two sets of entity-like objects
#objects must support the isCollision() interface
